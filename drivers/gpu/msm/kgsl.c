...
//1047PATCH CAF 12020515_ejkim77
#define FEATURE_QUALCOMM_BUG_FIX_LCD_1047_PATCH_ADD
...
#ifdef FEATURE_QUALCOMM_BUG_FIX_LCD_1047_PATCH_ADD			
...
#if 1//Qualcomm Patch //0001_1_Disable_slumbber_for_2D_engine.patch
...
#ifdef FEATURE_QUALCOMM_PERFORMANCE_PATCH
	kgsl_pwrctrl_wake(device);
#endif
...
error_put_file_ptr:
    switch (entry->memtype) {//kernel panic patch added by jiseunghwa
       case KGSL_MEM_ENTRY_PMEM:
       case KGSL_MEM_ENTRY_ASHMEM:
               if (entry->priv_data)
                       fput(entry->priv_data);
