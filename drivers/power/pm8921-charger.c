...
/* -------------------------------------------------------------------- */
#if defined(T_OSCAR)
#define OEM_MAX17058_STD_FULL		(1009)
#define OEM_MAX17058_STD_EMPTY	(-7)
#define OEM_MAX17058_EXT_FULL		(1015)
...
#elif defined(T_VEGAPVW)
#define OEM_MAX17058_STD_FULL		(1030)
#define OEM_MAX17058_STD_EMPTY	(13)
#define OEM_MAX17058_EXT_FULL		(1030)
#define OEM_MAX17058_EXT_EMPTY	(5)
#else
...
struct pm8921_chg_chip {
	...
	unsigned int update_cable_time;
	bool cable_usb_update;
	bool cable_dc_update; 
...
#if defined(CONFIG_PANTECH_CHARGER)
	temp = pm_chg_get_rt_status(chip, USBIN_OV_IRQ) && pm_chg_get_rt_status(chip, BATT_INSERTED_IRQ) &&
		!pm_chg_get_rt_status(chip, USBIN_UV_IRQ) && pm_chg_get_rt_status(chip,CHG_GONE_IRQ) &&
...
#if defined(FEATURE_ANDROID_PANTECH_USB_OTG_MODE)
...
#if defined(CONFIG_PANTECH_CHARGER)
	if (the_chip->cable_usb_update)
		__cancel_delayed_work(&the_chip->update_cable_work);

	the_chip->cable_usb_update = true;
	schedule_delayed_work(&the_chip->update_cable_work,
		round_jiffies_relative(msecs_to_jiffies(the_chip->update_cable_time)));
...
static void unplug_check_worker(struct work_struct *work)
{
	...
	if (usb_ma < 500 && !chip->cable_usb_suspend) {
            pm_chg_iusbmax_set(chip, 1); // set 500mA
            pm_chg_iusbmax_get(chip, &usb_ma);
...
static irqreturn_t chg_gone_irq_handler(int irq, void *data)
{
	...
	if (rc)
		pr_debug("Failed to read CHG_CNTRL_3 rc=%d\n", rc);
...
static void adjust_vdd_max_for_fastchg(struct pm8921_chg_chip *chip)
{
	...
	pm8921_bms_get_simultaneous_battery_voltage_and_current(&ichg_meas_ua,
								&vbat_uv);
	if (ichg_meas_ua >= 0) {
		pr_err("Exiting ichg_meas_ua = %d > 0\n", ichg_meas_ua);
		return;
	}
	if (ichg_meas_ua <= ichg_threshold_ua) {
		pr_err("Exiting ichg_meas_ua = %d < ichg_threshold_ua = %d\n",
...
static void update_cable(struct work_struct *work)
{
	...
	struct delayed_work *dwork = to_delayed_work(work);
	struct pm8921_chg_chip *chip = container_of(dwork, struct pm8921_chg_chip, update_cable_work);
...
static void __devinit determine_initial_state(struct pm8921_chg_chip *chip)
{
	...
	if (chip->usb_present ||chip->dc_present) {
		if (chip->usb_present) {
			chg_usb_type = USB_SDP_CHARGER;
			chip->cable_usb_update = true;
		} else if (chip->dc_present)
			chip->cable_dc_update = true;

		schedule_delayed_work(&chip->update_cable_work,
			round_jiffies_relative(msecs_to_jiffies(chip->update_cable_time)));
...
struct pm_chg_irq_init_data chg_irq_data[] = {
	...
	CHG_IRQ(USBIN_UV_IRQ, IRQF_TRIGGER_RISING | IRQF_TRIGGER_FALLING|IRQF_SHARED,
...
static int __devinit pm8921_charger_probe(struct platform_device *pdev)
{
...
#if defined(CONFIG_PANTECH_CHARGER)
	chip->update_cable_time = 0;
	chip->cable_usb_update = false;
	chip->cable_dc_update = false;
	chip->err_type = ERR_NONE;
	chip->err_fsm_type = FSM_STATE_OFF_0;
	chip->err_charge_done = false;
#endif
	...
	chip->wireless_psy.properties = pm_power_wireless_props,
	chip->wireless_psy.num_properties = ARRAY_SIZE(pm_power_wireless_props),
	chip->wireless_psy.get_property = pm_power_get_property,

	#if defined(PANTECH_POWER_SUPPLY_SYSFS_WRITE_TEST)
	chip->wireless_psy.set_property = pm_batt_power_set_property,
	#endif
#endif // #if defined(CONFIG_PANTECH_CHARGER_WIRELESS)
...
#if defined(CONFIG_PANTECH_CHARGER)
	INIT_DELAYED_WORK(&chip->update_cable_work, update_cable);
#endif
