...
static int mdp4_dtv_start(struct msm_fb_data_type *mfd)
{
	...
	MDP_OUTP(MDP_BASE + DTV_BASE, 0);  //mark. block DTV
#else
	/* enable DTV block */
...
{
/*PANTECH LS2 Do. layer1 added, for NULL pointer exception possibility*/
    if(dtv_pipe == NULL) {
	pr_err("%s: fail, dtv_pipe is NULL",__func__);
	return;
	}
	if (dtv_pipe->blt_addr) {
