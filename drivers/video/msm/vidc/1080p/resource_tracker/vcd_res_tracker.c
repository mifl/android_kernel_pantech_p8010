...
int res_trk_update_bus_perf_level(struct vcd_dev_ctxt *dev_ctxt, u32 perf_level)
{
	...
	if (perf_level <= RESTRK_1080P_VGA_PERF_LEVEL)
		...
		bus_clk_index = 1;
#if 0 // FEATURE_PANTECH_MMP_QCOM_CR P12554 Case(00846613)_while playing specific recoded clip, video frame is freezed for a
	else
		bus_clk_index = 2;
#else
	else if (perf_level <= RESTRK_1080P_1080P_PERF_LEVEL)
		bus_clk_index = 2;
	else
		bus_clk_index = 7;
...
#if 0 // FEATURE_PANTECH_MMP_QCOM_CR P12554 Case(00846613)_while playing specific recoded clip, video frame is freezed for a while
	if (dev_ctxt->reqd_perf_lvl + dev_ctxt->curr_perf_lvl == 0)
		bus_clk_index = 2;
	bus_clk_index = (bus_clk_index << 1) + (client_type + 1);
#endif
